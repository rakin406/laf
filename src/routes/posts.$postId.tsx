...
export const Route = createFileRoute("/posts/$postId")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Full details, photo and student comments for a reported AIUB lost item.",
      },
      { property: "og:title", content: "Lost item details — AIUB Lost & Found" },
      {
        property: "og:description",
        content: "See the photo, description and comments for this reported item.",
      },
    ],
  }),
  component: () => (
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5 py-2">
      ...
      {postQuery.isLoading ? (
        ...
        <ErrorState message={friendlyError(postQuery.error)} onRetry={() => void postQuery.refetch()} />
      ) : !postQuery.data ? (
        ...
        <>
          <PostCard
            post={postQuery.data}
            categoryLabel={labels.get(postQuery.data.category) ?? postQuery.data.category}
